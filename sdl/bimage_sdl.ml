...
let determine_format c =
  if c = 3 then
    if Sys.big_endian then Sdl.Pixel.format_bgr24
    else Sdl.Pixel.format_rgb24
  else
...
let make_surface image renderer =
  ...
  match Sdl.create_rgb_surface_with_format_from image.Image.data ~w ~h ~pitch ~depth f with
  | Ok surface ->
...
let create ?title flags image =
  (match Image.layout image with
  | Planar -> Error.exc (`Invalid_layout)
  | _ -> ());
  let w, h, _ = Image.shape image in
  match Sdl.create_window_and_renderer ~w ~h flags with
  | Ok (w, r) ->
      (match title with
      | Some t -> Sdl.set_window_title w t
      | None -> ());
