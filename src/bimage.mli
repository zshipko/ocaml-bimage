...
module Ffmpeg: sig
  ...
  val next :
    ?create:(string ->
            ?layout:Image.layout ->
            int -> int -> (int, u8, [> `Rgb ] as 'a) Image.t) ->
    ?layout:Image.layout -> t -> (int, u8, 'a) Image.t option

  (** Get the next frame *)

  val read_n:
    t ->
    ?create:(string -> ?layout:Image.layout ->  int -> int -> (int, u8, rgb) Image.t) ->
    int -> (int, u8, rgb) Input.t
