...
let to_color (Pixel data) =
  let kind = Data.kind data in
  let f x =
    (Kind.to_float kind x +. Kind.min_f kind)
    /. (Kind.max_f kind -. Kind.min_f kind)
  in
  Gg.Color.v
    (f (Bigarray.Array1.get data 0))
    (f (Bigarray.Array1.get data 1))
    (f (Bigarray.Array1.get data 2))
    1.0
...
let from_color ~dest px =
  ...
  let kind = Data.kind dest in
  if len >= 1 then
    Bigarray.Array1.set dest 0 (Kind.of_float kind (Gg.Color.r px));
  if len >= 2 then
    Bigarray.Array1.set dest 1 (Kind.of_float kind (Gg.Color.g px));
  if len >= 3 then
    Bigarray.Array1.set dest 2 (Kind.of_float kind (Gg.Color.g px));
  if len >= 4 then
    Bigarray.Array1.set dest 3 (Kind.of_float kind (Gg.Color.a px))
