...
module Input = struct
  ...
  let get_num_frames filename =
    let cmd =
      "ffmpeg -i \"" ^ filename
      ^ "\" -map 0:v:0 -c copy -f null -y /dev/null 2>&1 | grep frame= | tail \
         -n 1 | sed -n -e 's/^.*frame=//p' | awk '{ print $1 }'"
    in
  ...
  let create ?size filename =
    let width, height =
      match size with Some x -> x | None -> get_size filename
    in
    let ic =
  ...
  let close input = close_in input.pipe

  let read input =
    input.current_frame <- input.current_frame + 1;
    if input.current_frame >= input.num_frames then None
    else
...
module Output = struct
  type t = { filename : string; pipe : out_channel; width : int; height : int }
  ...
  let close output = close_out output.pipe
